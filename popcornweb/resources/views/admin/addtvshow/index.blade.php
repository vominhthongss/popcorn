	...
	<main class="main">
		<div class="container-fluid">
			<div class="row">
				...
				<!-- form -->
				<div class="col-12">
					<form action="{{ route('adminuploadtvshow') }}" class="form" method="POST" enctype="multipart/form-data">
                        @csrf
						<div class="row">
							...
							<div class="col-12 col-md-7 form__content">
								<div class="row">
                                    ...
                                    <div class="col-12 col-lg-6">
                                        <!-- Điểm IMDB -->
										<input type="number" min="1" max="10" step="0.1" name="imdb"  class="form__input" placeholder="Điểm IMDB"  required="required" pattern="[A-Za-z0-9]{1,20}">
									</div>
									<div class="col-12 col-lg-6">
                                        <!-- Thời lượng -->
								...
								<div class="row">
									...
									<div class="col-12">
										<div class="collapse show multi-collapse" id="multiCollapseExample1">
                                            ...
                                            <span class="form__title">Có {{$soTap}} tập</span>
                                            @for ($i=1;$i<=$soTap*3;$i+=3)
                                            ...
                                            <div class="form__video">
												<label id="caption{{($i+2)/3}}" for="form__caption{{($i+2)/3}}">Tải phụ đề</label>
												<input data-name="#caption{{($i+2)/3}}" id="form__caption{{($i+2)/3}}" name="caption[]" class="form__video-upload" type="file" accept=".vtt" required="required">
											</div>
                                            @else
                                            <div class="form__video">
												<label id="caption{{$i}}" for="form__caption{{$i}}">Tải phụ đề</label>
												<input data-name="#caption{{$i}}" id="form__caption{{$i}}" name="caption[]" class="form__video-upload" type="file" accept=".vtt" required="required">
											</div>
                                            @endif

                                            <div class="form__video">
												<label id="movie{{$i}}" for="form__video-upload{{$i}}">Tải phim 480p</label>
												<input data-name="#movie{{$i}}" id="form__video-upload{{$i}}" name="tvshow[]" class="form__video-upload" type="file" accept="video/mp4,video/x-m4v,video/*" required="required">
											</div>
                                            <div class="form__video">
												<label id="movie{{$i+1}}" for="form__video-upload{{$i+1}}">Tải phim 720p</label>
												<input data-name="#movie{{$i+1}}" id="form__video-upload{{$i+1}}" name="tvshow[]" class="form__video-upload" type="file" accept="video/mp4,video/x-m4v,video/*" required="required">
											</div>
                                            <div class="form__video">
												<label id="movie{{$i+2}}" for="form__video-upload{{$i+2}}">Tải phim 1080p</label>
												<input data-name="#movie{{$i+2}}" id="form__video-upload{{$i+2}}" name="tvshow[]" class="form__video-upload" type="file" accept="video/mp4,video/x-m4v,video/*" required="required">
											</div>
                                            @endfor
